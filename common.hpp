...
std::vector<cv::Point> plot_one_rotated_box(cv::Mat &img, float bbox[4], float theta){
    float r_w = Yolo::INPUT_W / (img.cols * 1.0);
    float r_h = Yolo::INPUT_H / (img.rows * 1.0);
    float gain;
    if(r_w > r_h)gain = r_h;
    else gain = r_w;
    float pad_w = (Yolo::INPUT_W - gain * img.cols) / 2;
    float pad_h = (Yolo::INPUT_H - gain * img.rows) / 2;
    ...
    std::cout<<"x_c,y_c,w,h,theta = "<<x_c<<" "<<y_c<<" "<<w<<" "<<h<<" "<<theta<<std::endl;
    cv::RotatedRect rRect = cv::RotatedRect(cv::Point2f(x_c, y_c), cv::Size2f(w, h), theta);
    ...
    for(int i=0;i<4;i++){
        ...
        vertices[i].x /= gain;
        vertices[i].y /= gain;
        cv::Point temp(vertices[i].x,vertices[i].y);
        contour.push_back(temp);
    }
    contours.push_back(contour);
    cv::drawContours(img,contours,-1,(120,0,250));
    return contour;
}
...
void longsideformat2poly(float bbox[4],float theta,std::vector<double> &bPoints)
{
    float x_c = bbox[0];
    float y_c = bbox[1];
    float w,h;
	...
	rRect.points(vertices);
    for(int i=0;i<4;i++){
        bPoints.push_back((double)vertices[i].x);
        bPoints.push_back((double)vertices[i].y);
    }
}

void rotate_nms(std::vector<Yolo::Detection>& res, float *output, float conf_thresh, float nms_thresh = 0.5) {
    int det_size = sizeof(Yolo::Detection) / sizeof(float);
    std::map<float, std::vector<Yolo::Detection>> m;
    for (int i = 0; i < output[0] && i < Yolo::MAX_OUTPUT_BBOX_COUNT; i++) {
        if (output[1 + det_size * i + 4] <= conf_thresh) continue;
    ...
    for (auto it = m.begin(); it != m.end(); it++) {
        ...
        for (size_t m = 0; m < dets.size(); ++m) {
            auto& item = dets[m];
            res.push_back(item);
            std::vector<double> bPoints0;
            
            longsideformat2poly(item.bbox,item.theta-179.9,bPoints0);
            for (size_t n = m + 1; n < dets.size(); ++n) {
                // iou_poly(rotatedRect,rotatedRect)
                std::vector<double> bPoints1;
                longsideformat2poly(dets[n].bbox,dets[n].theta-179.9,bPoints1);
                double iou = iou_poly(bPoints0,bPoints1) ;
                if (iou > nms_thresh) {
                    dets.erase(dets.begin() + n);
                    --n;
                }
    ...
    //     printf("bbox %f %f %f %f theta %f \n",res[i].bbox[0],res[i].bbox[1],res[i].bbox[2],res[i].bbox[3],res[i].theta);
    // }
}
